	...
	if (bWasSuccessful)
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Blue,
				FString::Printf(TEXT("Created session: %s , Num of session is: %d"), *SessionName.ToString(), OnlineSessionInterface->GetNumSessions())
			);
		}
		...
		if (World)
		{
			bool IsTravelSuccessful = World->ServerTravel(FString("/Game/ThirdPerson/Maps/Lobby?listen"));
